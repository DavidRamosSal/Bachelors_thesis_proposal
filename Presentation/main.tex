...
\begin{frame}{Formación estelar}
    ...
    \begin{minipage}{.52\linewidth}
        ...
        \captionof{figure}{Nebulosa del águila. Fuente: HST}
    \end{minipage}
    \end{figure}
\end{frame}
...
\begin{tabular}{ccccc}
\begin{tabular}[c]{@{}c@{}}Colapso del\\ núcleo de hierro\end{tabular} & $\Longrightarrow$ & \begin{tabular}[c]{@{}c@{}}Energía potencial\\  gravitacional es liberada\end{tabular} & $\Longrightarrow$ & \begin{tabular}[c]{@{}c@{}}Explosión de \\ la estrella\end{tabular}
\end{tabular}
\end{adjustbox}
\end{table}
...
\begin{frame}{Condicionando los posibles modelos \footnotemark}
    \footnotetext[2]{B. V. Ivanov. Analytical study of anisotropic compact star models.}
    \begin{itemize}
        \item \textbf{C1} \\
...
\begin{frame}[plain]{Planteamiento del problema}
    ...
    \end{figure}
    \centering
    %\Large Usar las condiciones de aceptabilidad para determinar si los modelos estáticos obtenidos con las ecuaciones de estado son útiles.
...
\centering
\vspace{-0.5cm}
\noindent\resizebox{0.53\textwidth}{!}{
\begin{ganttchart}[
...
%\begin{frame}[plain,noframenumbering,label=masa]{Sobre la función de masa}
 %   $m(r)$ \textbf{NO} es la suma de las energías propias de los elementos de fluido
  %  \begin{equation}
   %     4 \pi \int _ { 0 } ^ { r } \rho ( r ) e^{2\lambda}r ^ { 2 }     \dd{r}. \quad \hyperlink{back}{\beamerbutton{Back}}
%    \end{equation}
%\end{frame}
